...
need to do several operations on arrays, and hence this article.
...
assesssed by with the variable.
...
> after running **splice() **, it returns the removed item and removes it from the
> original array.
...
This method is similar to **splice()** but very different except that it returns
subarrays instead of substrings.
...
another example
    ...
    message.slice(0, 7); // returns "congrat"
...
another example:
    ...
    let firstName = 'b,o,l,a,j,i'
    firstName.split(',', 3); // ["b", "o", "l"]
...
It applies a function on all items in an array
    ...
    colors.forEach((item, index) => console.log(index, number));
...
There are more methods, this is just a few of them. Also, there are tons of
other actions that can be performed on arrays, try checking MDN docs
...
return a new array.
* **push()** adds items to the end of an array and **changes** the original array.
